...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => { // request is finished assembly the entire request body
    ...
    if (req.method === 'GET' && req.url.startsWith('/dogs/')) {
      ...
      res.statusCode = 404;
      res.setHeader('Content-Type', 'application/json');
      res.write(JSON.stringify({message: "This dog does not exist. Sorry!"}))
      return res.end();
    }
    ...
    if (req.method === 'DELETE' && req.url.startsWith('/dogs/')) {
      ...
      if (urlParts.length === 3) {
        ...
        const isIndex = el => String(el.dogId) === dogId;
        const dogIndex = dogs.findIndex(isIndex);
